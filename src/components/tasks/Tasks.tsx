...
const Tasks = () => {
    ...
    const [repeatPeriodFilter, setRepeatPeriodFilter] = React.useState<
        RepeatOptions | "ALL"
    >("ALL");
    const [completionFilter, setCompletionFilter] = React.useState<
        "ALL" | "YES" | "NO"
    >("ALL");
    ...
    const [noTasksMessage, setNoTasksMessage] = React.useState<string>(
    ...
    const checkFilter = (task: _Task) => {
        ...
        if (
            task.repeat_period !== repeatPeriodFilter &&
            repeatPeriodFilter !== "ALL"
        ) {
            return false;
        }
        ...
        if (
            (task.completed === true && completionFilter === "NO") ||
            (task.completed === false && completionFilter === "YES")
        ) {
            return false;
        }

        if (
            (task.date_time === true && dateFilter === "NODATE") ||
            (task.date_time === false && dateFilter === "DATE")
        ) {
            return false;
        }
            ...
            fetch(url("tracker") + "tasks/search", {
                ...
                .then((res) => {
                    if (!res.ok) {
                        ...
                        setNoTasksMessage(
                            "You do not have any tasks that match that query.",
                        );
                ...
                .catch(() => {
                    setAlert([
                        "An error occured while performing the search. Please try again.",
                        "ERROR",
                        true,
                    ]);
                    setTimeout(() => {
        ...
        <div className="page-parent">
            ...
            <ShowUser>
                <div className="flex flex-row flex-wrap">
                    ...
                    <input
                        className={
                            "form-input min-w-[350px] h-[50px] my-0 mr-[10px] mb-[10px] " +
                            (width < 800 ? "flex-grow" : "w-[70%]")
                        }
                ...
                <div className="w-full mb-[20px] mt-[10px] flex">
                    ...
                    >
                        <TabList className="flex w-full justify-between flex-wrap">
                            ...
                            <Tab className="tab fc group">
                                Never
                                <InformationCircleIcon
                                    className="size-5 ml-[5px] text-text"
                                    title="When a task that is set to never repeat is completed it will be removed from this page but you will be able to view it in your dashboard."
                                />
                            </Tab>
                    ...
                    >
                        <TabList className="flex w-full justify-between flex-wrap">
                            <Tab className="tab">All</Tab>
                            <Tab
                                className="tab disabled:hover:bg-white/5 disabled:bg-white/5 disabled:text-subtext"
                                disabled={repeatPeriodFilter === "NEVER"}
                            >
                                Completed
                            </Tab>
                    ...
                    >
                        <TabList className="flex w-full justify-between flex-wrap">
                            <Tab className="tab">All</Tab>
                            <Tab className="rounded-md py-1 px-3 text-sm focus:outline-none data-[selected]:bg-secondary data-[hover]:bg-secondarydark data-[selected]:white-border hover:white-border flex-grow min-w-[120px] h-[50px]">
                                Has Completion Date
                            </Tab>
                            <Tab className="rounded-md py-1 px-3 text-sm focus:outline-none data-[selected]:bg-secondary data-[hover]:bg-secondarydark data-[selected]:white-border hover:white-border flex-grow min-w-[120px] h-[50px]">
                                Doesn't Have Completion Date
                            </Tab>
                        </TabList>
                    </TabGroup>
                </div>
